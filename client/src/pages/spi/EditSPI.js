...
const EditSPI = () => {
  ...
  const handleSearch = async (e) => {
    ...
    if (!spiId.trim()) {
      setError('Please enter a SPI ID.');
      setFound(false);
      return;
    }
    ...
    try {
      const response = await apiClient.get(`/api/spi/${spiId.trim().toUpperCase()}`);
      
      if (response.data.success && response.data.data) {
        const spi = response.data.data;
        setFormData({
          totalpcba_allowed: spi.totalpcba_allowed || '',
          totalcycle_allowed: spi.totalcycle_allowed || ''
        });
        setFound(true);
        setError('');
      } else {
        setError('SPI not found.');
        setFound(false);
      }
    } catch (error) {
      setError(error.response?.data?.error || 'Error loading SPI: ' + error.message);
      setFound(false);
    } finally {
      setSearching(false);
    }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await apiClient.put(`/api/spi/edit/${spiId.trim().toUpperCase()}`, {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error!',
        text: error.response?.data?.error || 'Failed to update SPI: ' + error.message
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleSearch}>
        <table className="form-table">
          <tbody>
            <tr>
              <td className="label-cell">SPI ID:</td>
              <td>
                <input
                  type="text"
